...
export const BenefitStackResults = ({
  hooksData,
  hookRatings,
  setHookRatings,
  submissionData,
  ...
  const [mostRecentGenerationData, setMostRecentGenerationData] =
    useState<HookData | null>(null);
  ...
  const handleCopy = (hook: any) => {
    // const hookData = `${hook.n} - ${hook.a}`;
    const hookData = `${hook.a}`;
  ...
  useEffect(() => {
    if (hooksData.length > 0) {
      const latestGenerationData = hooksData[0];
      setMostRecentGenerationData(latestGenerationData);
    }
  }, [hooksData]);

  useEffect(() => {
    if (mostRecentGenerationData) {
      localStorage.setItem(
        "mostRecentGenerationData",
        JSON.stringify(mostRecentGenerationData)
      );
    }
  }, [mostRecentGenerationData]);
            ...
            >
              <div className="flex items-center justify-between p-2 md:mb-4 md:justify-start md:space-x-4 md:p-0">
                <h2 className="text-2xl font-semibold text-white text-opacity-90">
                  Your Benefit Stack #{generation.id}
                </h2>
