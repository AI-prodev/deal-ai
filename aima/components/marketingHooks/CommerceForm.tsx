...
const CommerceForm = ({
  ...
  const initialFormValues: FormValues = {
    ...
    triggerWords: "",
    imageDescription: "",
    imagePlacement: "On a simple rustic shelf with nothing else around it",
  };
  ...
  const tokenKey = "commerceHooksRequestToken";
  ...
  const handleEndResponseBenefit = (data: any) => {
    const response = data?.response;
    if (response) {
      const correlationId = response[0]?.input?.correlationId;
      const updatedHookData: Partial<CommerceHook> = {
  ...
  const {
    ...
    tokenKey: "productToken", // Unique token key for Product requests
  ...
  //start and track for product-placement
  const {
  ...
  useEffect(() => {
    ...
    const newGenerations = appData?.generations.map((gen: any) => {
      ...
      gen.contentItems.forEach((item: any) => {
        const content = item.contentId.map((cid: any) => {
          if (cid.output.h) return cid.output.h;
          if (cid.output.tag) return cid.output.tag;
          if (cid.output.product) return cid.output.product;
          if (cid.type === "benefit-stack" && cid.output.a)
            return { content: cid.output.a, id: cid._id };
          if (cid.output.q && cid.output.a)
            return { q: cid.output.q, a: cid.output.a };

          if (cid.type === "product-placement" && cid.output.url)
            return {
              content: cid.output.url,
              id: cid._id,
              editedUrl: cid?.output?.editedUrl?.url,
            };
          return null;
        });
        ...
        switch (item.name) {
          case "image":
            hookData.image.content = content[0]?.content;
            hookData.image.id = item.contentId[0]?._id;
            hookData.image.editedContent = content[0]?.editedUrl;
            break;
          case "faq":
            hookData.faq.content = content.map((item: any) => ({
              q: item.q,
              a: item.a,
            }));
            break;
          case "benefitStack":
            hookData.benefitStack.content = content;
            break;
          case "magicHook":
  ...
  return (
    <>
      <VisionSelector onTabChange={handleTabChange} defaultTabIndex={1} />
      <Formik
        ...
        {({ errors, submitCount, touched, values, setFieldValue }) => (
          <Form className="mt-5 space-y-5 text-white">
            ...
            {isLoadingBenefit || isLoadingFAQ ? (
              ...
              <button
                disabled={isLoadingBenefit || isLoadingFAQ || visionIsLoading}
                type="submit"
                className="btn btn-primary !mt-6 w-full"
              >
                {hooksData && hooksData.length >= 1
                  ? "Create Another Ecommerce PDP"
                  : "Create Ecommerce PDP"}
              </button>
            )}
          </Form>
        )}
