...
interface MostRecentImageItemProps {
  hook: any;
...
const MostRecentImageItem: React.FC<MostRecentImageItemProps> = ({
  hook,
  ...
  useEffect(() => {
    if (hook.editedUrl) {
      openEditedImagePreview(hook.editedUrl, hook.id, true);
    }
  }, [hook]);
    ...
    >
      {hook.editedUrl && (
        <button
          ...
          onClick={() => openEditedImagePreview(hook?.editedUrl ?? "", hook.id)}
      ...
      <div className="absolute left-3 top-3">
        <div className="rounded bg-black bg-opacity-60 p-2">
          <StarRating
            rating={hook.rating || 0}
            setRating={rating => handleRatingChange(hook.url, rating, hook.id)}
      ...
      <div className="absolute bottom-3 right-3 space-x-2">
        {isDownloadPopupVisible && selectedHookForDownload.id === hook.id && (
          <DownloadPopup
        ...
        <Tippy content="Edit" placement="top">
          <button
            ...
            onClick={() => handleEdit(hook as any, isEditedImagePreviewOpen)}
            disabled={uploadImageLoading}
          >
            <svg
              width="24"
        ...
        <Tippy content="Delete" placement="top">
          <button
            ...
            onClick={() => handleHookDeletionById(generationId, hook.id)}
          >
            <svg
              width="24"
              height="24"
...
export default MostRecentImageItem;
