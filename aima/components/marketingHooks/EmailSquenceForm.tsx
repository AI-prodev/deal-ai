...
import { ctaData, emailTypeData, languages } from "@/utils/data/Languages";
...
const EmailSequenceForm = ({
  ...
  const localStorageKeyForm = "emailFormValues";
  ...
  const [startEmailSequence, { isLoading: isStarting }] =
    useStartEmailSequenceRequestMutation();
  const [queryEmailSequence] = useQueryEmailSequenceRequestMutation();
  const [endEmailSequence] = useEndEmailSequenceRequestMutation();
  ...
  const { startAndTrack, isLoading, generationCount } = useServerTokenTracking({
    //@ts-ignore
    startRequest: startEmailSequence,
    //@ts-ignore
    queryRequest: queryEmailSequence,
    //@ts-ignore
    endRequest: endEmailSequence,
    tokenKey: tokenKey,
    ...
    const newGenerations = appData?.generations.map((gen: any) => {
      return {
        ...
        hooks: gen.creations.map((hook: any) => {
          const output = {
            body: hook.output?.body,
            subject: hook.output?.subject,
            url: hook.output?.url,
            timing: hook.output?.timing,
            id: hook._id,
            rating: hook.rating,
          };
      ...
      >
        {({ errors, submitCount, touched, values, setFieldValue }) => (
          <Form className="mt-5 space-y-5 text-white">
            ...
            <div className="mb-4">
              <Tippy content={"Abandoned Cart Sequence"} placement="top">
              ...
              <Field
                ...
                onChange={(e: ChangeEvent<HTMLSelectElement>) => {
                  const selectedEmailType = e.target.value;
                  setFieldValue("type", selectedEmailType);
                }}
              >
              ...
              {touched.type && errors.type ? (
                <div className="mt-1 text-danger">{errors.type}</div>
              ) : null}
            </div>
            ...
            <div className="mb-4">
              <Tippy
                ...
                placement="top"
              >
                <label htmlFor="cta" className="font-semibold text-white">
                  CTA
                </label>
              </Tippy>
              <Field
                  ...
                  >
                    <div className="space-y-6">
                      ...
                      <ToggleField
                        ...
                        label="Should the emails mention price?"
