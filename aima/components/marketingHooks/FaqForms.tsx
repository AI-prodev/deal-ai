...
const FaqForm = ({
  ...
  const localStorageKeyForm = "faqFormValues";
  ...
  const [startFaqRequest, { isLoading: isStarting }] =
    useStartFaqRequestMutation();
  ...
  const { startAndTrack, isLoading, generationCount } = useServerTokenTracking({
    //@ts-ignore
    startRequest: startFaqRequest,
    //@ts-ignore
    queryRequest: queryFaqRequest,
    //@ts-ignore
    endRequest: endFaqRequest,
    tokenKey: tokenKey,
