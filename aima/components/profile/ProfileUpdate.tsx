...
const ProfileUpdate = ({}: ProfileUpdateProps) => {
  ...
  const [createOrUpdateProfile] =
    profileApiClient.useCreateOrUpdateProfileMutation();
  ...
  useEffect(() => {
    if (!isLoading && profile && profile.fields) {
      setInitialValues({
  ...
  const { initials, bgColor } = getInitialsAndColor(initialValues.firstName);

  const userId = session?.id; // Get the user ID from the session data
        ...
        >
          {({
            ...
            <Form className="space-y-5">
                ...
                onChange={e => {
                  if (e.target.files && e.target.files[0]) {
                    ...
                    reader.onload = event => {
                      ...
                      if (imgElement) {
                        (imgElement as HTMLImageElement).src = event.target
                          ?.result as string;
                      }
                    };
              ...
              />
              {initialValues.profileImage || imageFile ? (
                <img
                  src={initialValues.profileImage}
                  alt="img"
                  className="profile-image mb-5 h-24 w-24 cursor-pointer rounded-full object-cover"
                  onClick={() => {
                ...
                <div
                  className={`mb-5 flex h-24 w-24 cursor-pointer  items-center justify-center rounded-full text-5xl text-white ${bgColor}`}
                  onClick={() => {
                    fileInputRef.current?.click();
                  }}
                >
              ...
              {/* <div className="grid grid-cols-1 gap-5 md:grid-cols-4">
                ...
                <div
                  className={touched.state && errors.state ? "has-error" : ""}
                >
                ...
                <div className={touched.zip && errors.zip ? "has-error" : ""}>
              ...
              <div className={touched.phone && errors.phone ? "has-error" : ""}>
              ...
              <div className="mt-3 mb-3 sm:col-span-2">
                <button
                  disabled={isLoading}
                  type="submit"
                  className="btn btn-primary"
                >
                  {isLoading || (isSubmitting && <LoadingSpinner isLoading />)}
                  Save
                </button>
              </div>
            </Form>
          ...
          <div className="mt-10">
            ...
            <div>
              <button
                type="button"
                className={`btn btn-primary`}
                onClick={() => router.push("/apps/settings/manage")}
              >
                <div className="flex items-center">
                  ...
                  <span className="text-white ltr:pl-3 rtl:pr-3 dark:text-white dark:group-hover:text-white-dark">
                    Manage Subscription
                  </span>
                </div>
              </button>
