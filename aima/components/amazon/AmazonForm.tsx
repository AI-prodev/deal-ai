...
const AmazonForm = ({
  ...
  const tokenKey = "amazonHooksRequestToken";
  ...
  const handleEndResponseBenefit = (data: any) => {
    ...
    if (response) {
      ...
      const updatedHookData: Partial<CommerceHook> = {
        id: correlationId,
        benefitStack: {
          content: response.map((item: any) => item.a),
          isLoading: false,
        },
      };
  ...
  const {
    ...
    tokenKey: "amazonBenefitStackToken",
  ...
  const {
    ...
    tokenKey: "amazonSeoToken", // Unique token key for SEO requests
  ...
  const {
    ...
    tokenKey: "amazonProductToken", // Unique token key for Product requests
  ...
  const {
    ...
    tokenKey: "amazonProductPlacementToken",
    ...
    <>
      ...
      >
        {({ errors, submitCount, touched, values, setFieldValue }) => (
          <Form className="space-y-5 text-white mt-5">
            {selectedTab === "Text" && (
              <>
                <div className="mt-4">
            ...
            {isLoadingBenefit ? (
              ...
              <button
                disabled={isLoadingBenefit || visionIsLoading}
                type="submit"
                className="btn btn-primary !mt-6 w-full"
              >
                {hooksData && hooksData.length >= 1
                  ? "Create Another Amazon PDP"
                  : "Create Amazon PDP"}
              </button>
            )}
          </Form>
