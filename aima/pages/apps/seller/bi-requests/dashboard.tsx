...
const BusinessInformationRequestsDashboard = () => {
  useAuth();
  ...
  const {
    useGetBusinesInformationRequestsBySellerIdQuery,
    useGetBusinesInformationRequestsByIdQuery,
    usePatchBusinessRequestInformationMutation,
  } = api;

  const {
    data: sellerBusinessList,
    isError,
    isFetching,
    error,
  } = useGetBusinesInformationRequestsBySellerIdQuery(sessionData?.id || "");

  const {
    data: editingBusiness,
    isError: isEditingBusinessWithError,
    isLoading: isEditingBusinessLoading,
    error: editingBusinessError,
  } = useGetBusinesInformationRequestsByIdQuery(editingBusinessId || skipToken);

  const [queryRequest, { isLoading }] =
    usePatchBusinessRequestInformationMutation();

  const appName = "Business Request Information Dashboard";

  useEffect(() => {
    if (editingBusinessId !== "") {
      router.replace(
        `/apps/seller/bi-requests/dashboard?id=${editingBusinessId}`
      );
    }
  }, [editingBusinessId]);

  useEffect(() => {
    if (!isEditingBusinessLoading && !isEditingBusinessWithError) {
      setResponses(() => {
        if (
          editingBusiness?.response?.responses &&
          editingBusiness?.response?.responses?.length > 0
        ) {
          return editingBusiness?.response?.responses;
        } else {
          return transformMarkdownToArrayOfObjects(
            editingBusiness?.response?.checklist ?? ""
          );
        }
      });
    }
      ...
      <div>
        ...
        <div className="space-y-2 pt-5">
          <h5 className="text-lg font-semibold dark:text-white-light">
            {!editingBusiness
              ? appName
              : editingBusiness.response.business?.businessName +
                " - " +
                `${editingBusiness.response.buyer?.firstName} ${editingBusiness.response.buyer?.lastName}` +
                " - Edit"}
          </h5>
          {!editingBusinessId ? (
            isError ? (
              ...
              sellerBusinessList?.response.map(requestsList => (
                <BusinessTable
                  key={requestsList.businessName}
                  business={requestsList}
                  callback={setEditingBusinessId}
                />
              ))
            )
          ) : (
...
export default withAuth(BusinessInformationRequestsDashboard, SELLER_ROLES);
