...
const CommerceHooks = (props: Props) => {
  ...
  const [hooksData, setHooksData] = useState<CommerceHook[]>(() => {
    const localData = localStorage.getItem("commerceHooksData");
    return localData ? JSON.parse(localData) : [];
  });

  // useEffect(() => {
  //   localStorage.setItem("commerceHooksData", JSON.stringify(hooksData));
  // }, [hooksData]);

  useEffect(() => {
    hooksData.forEach(async hook => {
      const allLoaded =
        !hook.image?.isLoading &&
        ...
        <div className="flex justify-center">
          <div className="my-3 w-full max-w-[780px] pt-2">
            <div className="mb-8 flex items-center">
              <h2 className="text-2xl font-bold">Ecommerce PDP</h2>
              <div className="mx-4">
