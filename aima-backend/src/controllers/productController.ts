...
import { processRequestProduct } from "../utils/processRequestProduct"

export const startProductRequest = async (req: Request, res: Response) => {
    ...
    processRequestProduct(token, input, userId).catch(async (err) => {
        console.error(err)
        await setRedis(
            token,
            JSON.stringify({
...
export const endProductRequest = async (
    ...
    if (request.status === "completed") {
        ...
        const slicedContent = parsedContent.slice(0, 10)
        ...
        if (input?.correlationId) {
            await updateOrCreateComposite(
                `correlationId:${input.correlationId}`,
                {
                    input: input,
                    output: {
                        product: slicedContent.slice(0, 10)
                    }
                }
            )
        }
...
export const queryProductRequest = async (req: Request, res: Response) => {
    const token = req.params.token
    const request = JSON.parse(await getRedis(token))
