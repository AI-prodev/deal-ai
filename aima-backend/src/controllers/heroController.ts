...
import { processHeroRequest } from "../utils/processRequestHero"
import { getRedis, setRedis, deleteRedis } from "../services/redis.service"

export const startHeroRequest = async (req: Request, res: Response) => {
    ...
    processHeroRequest(input, token, userId).catch(async (err) => {
...
export const endHeroRequest = async (
...
export const queryHeroRequest = async (req: Request, res: Response) => {
    const token = req.params.token
    const request = JSON.parse(await getRedis(token))
